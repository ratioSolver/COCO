...
impl<DB: Database + Send + Sync, KB: KnowledgeBase + Send> CoCo<DB, KB> {
    ...
    pub async fn create_class(&self, name: &str, parents: Option<HashSet<String>>, static_properties: Option<HashMap<String, Property>>, dynamic_properties: Option<HashMap<String, Property>>) {
    ...
    fn add_rules(&self, db_rules: Vec<Rule>) {
        for rule in db_rules {
            self.kb.lock().unwrap().create_rule(&rule).expect("Failed to create rule in knowledge base");
