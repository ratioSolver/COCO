...
namespace coco
{
    ...
    {
        coco_db::set_sensor_data(s, timestamp, val);
        auto result = sensor_data_collection.insert_one(bsoncxx::builder::stream::document{} << "sensor_id" << bsoncxx::oid{bsoncxx::stdx::string_view{s.get_id()}} << "timestamp" << bsoncxx::types::b_date{timestamp} << "value" << bsoncxx::from_json(val.to_string()) << bsoncxx::builder::stream::finalize);
        if (result)
            coco_db::set_sensor_data(s, timestamp, val);
    }
    ...
    parameter_ptr mongo_db::to_par(const bsoncxx::v_noabi::document::view &doc)
    {
        ...
        case parameter_type::Integer:
        {
            ...
            if (doc.find("min") != doc.end())
                switch (doc["min"].type())
                {
                case bsoncxx::type::k_int32:
                    min = doc["min"].get_int32().value;
                    break;
                case bsoncxx::type::k_int64:
                    min = doc["min"].get_int64().value;
                    break;
                case bsoncxx::type::k_double:
                    min = doc["min"].get_double().value;
                    break;
                default:
                    assert(false);
                }
            long max = std::numeric_limits<long>::max();
            if (doc.find("max") != doc.end())
                switch (doc["max"].type())
                {
                case bsoncxx::type::k_int32:
                    max = doc["max"].get_int32().value;
                    break;
                case bsoncxx::type::k_int64:
                    max = doc["max"].get_int64().value;
                    break;
                case bsoncxx::type::k_double:
                    max = doc["max"].get_double().value;
                    break;
                default:
                    assert(false);
                }
