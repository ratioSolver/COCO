...
namespace coco
{
    ...
    void coco_db::create_user(std::string_view itm_id, std::string_view username, std::string_view password, json::json &&personal_data)
    {
        LOG_WARN("Creating user " + std::string(username) + " with password " + std::string(password) + " and personal data " + personal_data.dump() + " for item " + std::string(itm_id));
    }
#endif
    ...
    void coco_db::create_type(std::string_view tp_name, const std::vector<std::string> &parents, const json::json &data, const json::json &static_props, const json::json &dynamic_props)
    {
    ...
    void coco_db::set_parents(std::string_view tp_name, const std::vector<std::string> &parents)
    {
        if (!parents.empty())
        {
            LOG_WARN(std::string("Setting parents for ") + tp_name.data());
            std::string pars_str;
            for (auto p_i = parents.begin(); p_i != parents.end(); ++p_i)
            {
                if (p_i != parents.begin())
            ...
            LOG_WARN(std::string("Deleting parents for ") + tp_name.data());
    }
