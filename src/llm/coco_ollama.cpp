...
namespace coco
{
    coco_ollama::coco_ollama(coco &cc, std::string_view host, unsigned short port, std::string_view model) noexcept : coco_llm(cc), client(host, port), async_client(), session(async_client.get_session(host, port)), model(model) {}

    std::string coco_ollama::understand(std::string_view message) noexcept
    {
