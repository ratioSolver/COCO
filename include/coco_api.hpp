...
namespace coco
{
  ...
  const json::json coco_paths{
      ...
      {"/user",
       {{"post",
         {{"summary", "Create a new user"},
          ...
          {"requestBody",
           {{"content", {{"application/json", {{"schema", {{"type", "object"}, {"properties", {{"username", {{"type", "string"}}}, {"password", {{"type", "string"}}}, {"personal_data", {{"type", "object"}}}, {"data", {{"type", "object"}}}}}}}}}}},
            {"required", true}}},
          {"responses",
           {{"200",
      ...
      {"/user/{user_id}",
       {{"get",
         {{"summary", "Retrieve a user personal data"},
          {"description", "Endpoint to fetch the personal data of a user"},
          {"security", std::vector<json::json>{{"bearerAuth", std::vector<json::json>{}}}},
          {"parameters",
           {{{"name", "user_id"},
             {"in", "path"},
          ...
          {"responses",
           {{"200",
             {{"description", "Successful response with the user personal data"},
  ...
  const json::json geometry_schema{
      {"point",
       {{"type", "object"},
        {"properties",
         {{"type", {{"type", "string"}, {"enum", {"Point"}}}},
          {"coordinates", {{"type", "array"}, {"minItems", 2}, {"items", {{"type", "number"}}}}}}},
        {"required", std::vector<json::json>{"type", "coordinates"}}}},
      {"line_string",
       {{"type", "object"},
        {"properties",
      ...
      {"geometry",
       {{"oneOf",
         std::vector<json::json>{{"$ref", "#/components/schemas/point"},
