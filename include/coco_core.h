...
namespace coco
{
  ...
  private:
    ...
    COCO_EXPORT static std::string value_to_string(const json::json &value, const std::string &prefix = "")
    {
      std::string str;
      for (const auto &[id, val] : value.get_object())
        if (val.get_type() == json::json_type ::object)
          str += ' ' + value_to_string(val, prefix + id + '.');
        else
          str += ' ' + prefix + id + ' ' + val.to_string();
      return str;
    }
