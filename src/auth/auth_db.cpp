...
namespace coco
{
    auth_db::auth_db(mongo_db &db, std::string_view mongodb_users_uri) noexcept : mongo_module(db), users_conn(mongocxx::uri(mongodb_users_uri.data())), users_db(users_conn[db.get_config()["name"].get<std::string>() + "_users"]), users_collection(users_db["users"])
    {
        assert(users_conn);
