...
=>
    ...
    (if (and ?parkinson ?baseline_freezing) then (bind ?anxiety_relevant (append$ ?anxiety_relevant baseline_freezing)))
    (if (and ?parkinson (>= ?recent_freezing_episodes 3)) then (bind ?anxiety_relevant (append$ ?anxiety_relevant recent_freezing_episodes)))
    (if (and (or ?parkinson ?psychiatric_patients ?multiple_sclerosis ?young_pci_autism) (>= ?heart_rate 100)) then (bind ?anxiety_relevant (append$ ?anxiety_relevant heart_rate)))
    (if (and (or ?psychiatric_patients ?young_pci_autism) (>= ?baseline_heart_rate 100)) then (bind ?anxiety_relevant (append$ ?anxiety_relevant baseline_heart_rate)))
    (if (and (or ?psychiatric_patients ?young_pci_autism ?parkinson ?multiple_sclerosis) (>= ?heart_rate_differential 50)) then (bind ?anxiety_relevant (append$ ?anxiety_relevant heart_rate_differential)))
    (if (and (or ?parkinson ?psychiatric_patients ?older_adults ?multiple_sclerosis ?young_pci_autism) (>= ?respiratory_rate 30)) then (bind ?anxiety_relevant (append$ ?anxiety_relevant respiratory_rate)))
    (if (and ?psychiatric_patients (>= ?galvanic_skin_response 50)) then (bind ?anxiety_relevant (append$ ?anxiety_relevant galvanic_skin_response)))
    (if (and ?older_adults (>= ?high_blood_pressure 100)) then (bind ?anxiety_relevant (append$ ?anxiety_relevant high_blood_pressure)))
    (if (and ?multiple_sclerosis ?self_perception) then (bind ?anxiety_relevant (append$ ?anxiety_relevant self_perception)))
